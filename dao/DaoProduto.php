...
class DaoProduto {
  ...
  public function porId(int $id): ?Produto {
    $sql = "SELECT produtos.nome, produtos.preco, 
                   produtos.estoque, produtos.marca_id, marcas.nome, embalagem_nome, embalagem_id
            FROM produtos 
            LEFT JOIN marcas ON marcas.id = produtos.marca_id
            LEFT JOIN embalagens ON emebalagens.id = produtos.embalagem_id
            WHERE produtos.id = ?";
    ...
    if ($stmt) {
      ...
      if ($stmt->execute()) {
        $stmt->bind_result($nome,$preco,$estoque,$marca_id,$marca_nome,$embalagem_id,$embalagem_nome);
        $stmt->store_result();
        if ($stmt->num_rows == 1 && $stmt->fetch()) {
          $prod = new Produto($nome, $preco, $estoque, 
            new Marca($marca_nome, $marca_id),new Embalagem($embalagem_nome, $embalagem_id), $id);
        }
      }
      $stmt->close();
    }
  ...
  public function inserir(Produto $produto): bool {
    $sql = "INSERT INTO produtos (nome,preco,estoque,marca_id,embalagem_id) VALUES(?,?,?,?)";
    $stmt = $this->connection->prepare($sql);
    $res = false;
    if ($stmt) {
      ...
      $embalagem_id = $produto->getembalagem()->getId();
      $stmt->bind_param('sdii', $nome, $preco, $estoque, $marca_id, $embalagem_id);
      if ($stmt->execute()) {
  ...
  public function atualizar(Produto $produto): bool {
    $sql = "UPDATE produtos SET nome=?, preco=?, estoque=?, marca_id=?, embalagem_id WHERE id = ?";
    $stmt = $this->connection->prepare($sql);
    $ret = false;      
    if ($stmt) {
      $nome = $produto->getNome();
      ...
      $embalagem_id = $produto->getEmbalagem()->getId();      
      $id   = $produto->getId();
      $stmt->bind_param('sdiii', $nome, $preco, $estoque, $marca_id, $embalagem_id, $id);
      $ret = $stmt->execute();
      $stmt->close();
    }
    return $ret;
  ...
  public function todos(): array {
    $sql = "SELECT produtos.id, produtos.nome, produtos.preco, 
            ...
            FROM produtos 
            LEFT JOIN marcas ON marcas.id = produtos.marca_id
            LEFT JOIN embalagens ON embalagens.id = produtos.embalagem_id";
    $stmt = $this->connection->prepare($sql);
    $produtos = [];
    if ($stmt) {
      if ($stmt->execute()) {
        $id = 0; $nome = '';
        $stmt->bind_result(
          $id, $nome, $preco, $estoque, $marca_id, $marca_nome, $embalagem_id, $embalagem_nome
        );
        $stmt->store_result();
        while($stmt->fetch()) {
          ...
          $embalagem = new Embalagem ($embalagem_nome, $embalagem_id);
          $produtos[] = new Produto($nome, $preco, $estoque, $marca, $embalagem, $id);
        }
      }
      $stmt->close();
    }
